...
public class FileAccess
{
	...
	public void Write(String strInputContent, int iType) throws IOException
	{
		...
		switch (iType)
		{
			case ConstantDefine.FileWriteType.ADD: // 追加新的内容
				...
				if (!whetherOver)
				{
					...
					{
						// 如果还没有该项属性 在末尾添加该项属性
						String add = System.getProperty("line.separator")
						        + System.getProperty("line.separator")
						        + fileProp.getName() + "#"
						        + fileProp.getDescrp();
			...
			default:
				...
				if (iSameKeyCount == 0)
				{
					...
					String add = System.getProperty("line.separator")
					        + System.getProperty("line.separator")
					        + fileProp.getName() + "#" + fileProp.getDescrp();
					add += System.getProperty("line.separator")
					        + strInputContent;
					exist[exist.length - 1] = exist[exist.length - 1] + add;
				}
		...
		if (!fileExist || !head)
			writeStr = fileHead + System.getProperty("line.separator")
			        + System.getProperty("line.separator") + strContent;
		else
			writeStr = strContent;
